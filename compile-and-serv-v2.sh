...
# Fedora's own gcc-arm-linux-gnu package. Despite the "gnueabi" name (no
# "hf") it's configured --with-float=hard --with-abi=aapcs-linux, matching
# this target exactly. Board-verified 2026-09-06: full zImage/uImage/modules
# build, boots, fan/LEDs/network/PIC all work identically to the old
# toolchain. It's kernel-only (--with-newlib --without-headers, no target
# glibc) -- fine here since kernel builds need no libc, but it cannot build
# standalone userspace tools like tools/qnap-ts228/uart_probe.c; those still
# need a full sysroot toolchain such as the Linaro one kept below.
...
apply_debug_config() {
	# DEBUG_LL ("kernel low-level debugging functions") is a pre-console
	# early-boot debug aid, not needed for normal operation -- this board
	# has a fully working serial console via SERIAL_8250_DW, and bootargs
	# already carry earlycon=uart8250,mmio32,0x18007800 for anything that
	# has to print before that comes up. earlycon is the mechanism to
	# reach for on a multiplatform kernel; DEBUG_LL pins the image to one
	# hardcoded UART and should stay off.
	#
	# It used to fail the build outright: the vendor tree re-added a
	# DEBUG_LL_UART_NONE choice entry that mainline had removed, so with
	# DEBUG_LL=y and no UART selected, DEBUG_LL_INCLUDE fell through to
	# its last-resort default "mach/debug-macro.S" -- a file no longer
	# provided once mach-rtd119x/ was retired. arch/arm/Kconfig.debug is
	# back to pristine now, so enabling DEBUG_LL resolves to a real UART
	# option and builds; this stays as a belt-and-braces default-off.
	./scripts/config --disable DEBUG_LL
	./scripts/config --disable DEBUG_UNCOMPRESS
	make olddefconfig
}
...
apply_eeprom_config() {
	# 24c04-class manufacturing EEPROM at 0x50 on the same I2C bus as the
	# RX-8010 RTC (see the board dts) -- vendor/model strings, PCB part
	# number, and the unit serial number. Generic driver, no board glue.
...
apply_armv7_neon_config() {
	...
	./scripts/config --enable CRYPTO_DEV_RTK_MCP
	make olddefconfig
}
