...
//	struct device *device;
	...
	int (*get_settings)(struct net_device *, struct ethtool_link_ksettings *);
...
static int rtl8169_gset_tbi(struct net_device *dev,
			    struct ethtool_link_ksettings *cmd)
{
	...
	supported = SUPPORTED_1000baseT_Full | SUPPORTED_Autoneg | SUPPORTED_FIBRE;
...
static int rtl8169_gset_xmii(struct net_device *dev,
			     struct ethtool_link_ksettings *cmd)
{
	struct rtl8169_private *tp = netdev_priv(dev);
...
{
	/*
	 * CSI programs PCIe config-space quirks (ASPM latency etc.). This
	 * platform GMAC has no working CSI engine, so the FLAG handshake in
	 * CSIAR never completes and rtl_csiar_cond timeouts flood the
	 * console. Leave ops NULL; rtl_csi_read/write become no-ops.
	 */
	tp->csi_ops.write = NULL;
	tp->csi_ops.read = NULL;
}
...
rtl_init_one(struct platform_device *pdev)
{
	...
	if (IS_ERR(rstc_etn)) {
		printk(KERN_ERR "%s: reset_control_get_exclusive() returns %ld\n", __func__, PTR_ERR(rstc_etn));
		rstc_etn = NULL;
	}
