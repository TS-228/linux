...
//	struct device *device;
	...
	struct mdio_ops {
		void (*write)(struct rtl8169_private *tp, int reg, int value);
		int (*read)(struct rtl8169_private *tp, int reg);
	} mdio_ops;
...
MODULE_DESCRIPTION("Realtek RTD1195 SoC Gigabit Ethernet driver");
...
/*
 * phy_connect_direct() callback: phylib has already updated netif_carrier
 * by the time this runs, so this only drives the platform-specific side
 * effects that used to live in the raw link_ok() polling path.
 */
static void r8169_phylink_handler(struct net_device *dev)
{
...
static void __rtl8169_set_wol(struct rtl8169_private *tp, u32 wolopts)
{
	...
	/* The device tree pins this SoC to RTL_GIGA_MAC_VER_42. */
...
{
	/*
	 * Unlike the PCIe r8169 family, RTD1195's integrated MAC isn't
	 * behind a TxConfig register that encodes a chip revision, so
	 * there is nothing to probe here beyond the family default.
	 */
...
{
	struct rtl8169_private *tp = timer_container_of(tp, t, timer);
...
/*
 * Registers the internal PHY as an mii_bus device so phylib can drive it.
 * The register access itself is unchanged -- same PHYAR poll-and-delay
 * sequence as before, just reached through mii_bus->read/write instead of
 * struct mii_if_info.
 */
static int r8169_mdio_register(struct rtl8169_private *tp)
{
	...
	new_bus->name = "r8169soc";
	new_bus->priv = tp;
	new_bus->parent = &pdev->dev;
	new_bus->irq[0] = PHY_MAC_INTERRUPT;
	new_bus->phy_mask = GENMASK(31, 1);
	snprintf(new_bus->id, MII_BUS_ID_SIZE, "r8169soc-%s", dev_name(&pdev->dev));
	...
	if (!tp->phydev) {
		...
		/*
		 * The internal PHY reports RTL_GENERIC_PHYID and needs
		 * drivers/net/phy/realtek.c's "Generic FE-GE Realtek PHY"
		 * entry bound -- genphy alone does not work for it.
		 */
		dev_err(&pdev->dev,
			"no dedicated PHY driver found for PHY ID 0x%08x, is CONFIG_REALTEK_PHY enabled?\n",
			tp->phydev->phy_id);
		return -EUNATCH;
	}
...
static void rtl_hw_start_8168g_1(struct rtl8169_private *tp)
{
	...
	/*
	 * On PCI/PCIe r8169 this tunes the Max Read Request Size via PCIe
	 * config space; RTD1195's MAC has no PCIe link to negotiate, so
...
static void rtl8169_tx_desc_dump(struct rtl8169_private *tp)
{
	...
	for (i = 0; i < NUM_TX_DESC; i++)
		pr_info("Desc[%d] opts1=%x opts2=%x addr= %llx\n", i, tp->TxDescArray[i].opts1,
			tp->TxDescArray[i].opts2, tp->TxDescArray[i].addr);
}
...
static void rtl_reset_work(struct rtl8169_private *tp)
{
	...
	// prevent Tx desc is overwritten
				      ...
				      struct net_device *dev)
{
	...
	/* mmiowb() removed on ARM; wmb above already orders MMIO */
...
static void rtl_slow_event_work(struct rtl8169_private *tp)
{
	...
	if (status & LinkChg) {
		...
		/*
		 * Prevent ALDPS from entering MAC Powercut Tx/Rx disable;
		 * use a MAC reset to set the counter back to offset 0.
		 */
		if (tp->link_ok(ioaddr))
			rtl_schedule_task(tp, RTL_FLAG_TASK_RESET_PENDING);
		else
...
//	pm_runtime_get_sync(&pdev->dev);
	...
	retval = phy_connect_direct(dev, tp->phydev, r8169_phylink_handler,
				    tp->supports_gmii ? PHY_INTERFACE_MODE_GMII :
	...
	if (!tp->supports_gmii)
		phy_set_max_speed(tp->phydev, SPEED_100);
	phy_attached_info(tp->phydev);
...
static void __rtl8169_resume(struct net_device *dev)
{
	...
	pr_info("resume r8169 eth0 gphy work\n");
...
{
	/* The 8168b WoL receiver quirk does not apply to this chip. */
}
...
static unsigned int rtl_try_msi(struct rtl8169_private *tp,
				const struct rtl_cfg_info *cfg)
{
	...
	/*
	 * RTD1195's MAC is an integrated platform device, not on a PCI
	 * bus, so there is no pci_dev to negotiate MSI through here.
	 */
...
static ssize_t read_proc(struct file *filp, char *buf, size_t count, loff_t *offp)
{
	...
	pr_info("read procfs ethernet wol_enable = %x\n", wol_enable);
...
static ssize_t write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	...
	wol_enable = val;
	pr_info("write procfs ethernet wol_enable = %x\n", wol_enable);
...
rtl_init_one(struct platform_device *pdev)
{
	...
	struct reset_control *rstc_etn = reset_control_get_exclusive(&pdev->dev, "rst_etn");

	if (IS_ERR(rstc_etn)) {
		pr_err("%s: reset_control_get_exclusive() returns %ld\n", __func__, PTR_ERR(rstc_etn));
		rstc_etn = NULL;
	}
	...
	if (!ioaddr) {
		pr_err("%s: failed to map MMIO region\n", __func__);
		rc = -ENOMEM;
		goto out;
	}
	...
	do {
		...
		rtw_proc = proc_mkdir_data("eth0",  0555, init_net.proc_net, NULL);
		...
		if (!tp->dir_dev) {
			tp->dir_dev = proc_mkdir_data(MODULENAME, S_IFDIR | 0555, rtw_proc, NULL);
			dir_dev = tp->dir_dev;
		...
		entry = proc_create_data("wol_enable", S_IFREG | 0444,
	...
	if (!of_get_mac_address(pdev->dev.of_node, mac_addr)) {
		...
		/* RTD119x MAC0 readback may not match programmed RAR. */
		eth_hw_addr_set(ndev, mac_addr);
	} else {
		u8 addr[ETH_ALEN];

		for (i = 0; i < ETH_ALEN; i++)
			addr[i] = RTL_R8(MAC0 + i);
	...
	netif_napi_add_weight(ndev, &tp->napi, rtl8169_poll, R8169_NAPI_WEIGHT);

	/*
	 * don't enable SG, IP_CSUM and TSO by default - it might not work
	 * properly for all devices
	 */
	ndev->features |= NETIF_F_SG | NETIF_F_IP_CSUM | NETIF_F_TSO | NETIF_F_RXCSUM |
