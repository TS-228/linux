				      ...
				      unsigned long action, void *data)
{
	...
	case CPUFREQ_PRECHANGE:
		...
		sscd->cpu = freqs->policy->cpu;
		break;

	case CPUFREQ_POSTCHANGE:
		if (freqs->policy->cpu != sscd->cpu)
			return NOTIFY_DONE;
		...
		dev_dbg(sscd->dev, "%s: cpu%d: new_freq=%d, old_volt=%d\n",
			__func__, freqs->policy->cpu, new_freq, old_volt);
...
static int scpu_sramctl_init(struct device *dev)
{
	struct scpu_sramctl_data *sscd;
	struct regulator *supply;
	int ret;

	if (WARN_ON_ONCE(sscdp))
		return -EBUSY;
...
static const struct of_device_id scpu_sramctl_ids[] = {
