...
static char *rpc_devnode(const struct device *dev, umode_t *mode)
{
	*mode = 0666;
	return NULL;
}
...
static ssize_t kernel_remote_allocate_show(const struct class *class, const struct class_attribute *attr, char *buf)
{
...
static void __maybe_unused rtk_rpc_remove(struct platform_device *pdev)
{
}
