...
#define COLUMN 3
			...
			if ((prop) && (!err)) {
				...
				for (i=0; i<counter; i+=1) {
					...
					pool->base  = (phys_addr_t)((u32)of_read_number(prop, 1 + (i*COLUMN)));
			...
			if (heap_data->priv) {
				...
				list_for_each_entry(pool, pools, list) {
					if (pool->base == 0 && pool->type == RTK_CARVEOUT_GEN_POOL_TYPE) {
						struct page *page = dma_alloc_from_contiguous(&(pdev->dev), pool->size >> PAGE_SHIFT, get_order(pool->size), GFP_KERNEL);
...
static void rtk_ion_remove(struct platform_device *pdev)
{
