...
static int RTK_pctrl_get_groups_count(struct pinctrl_dev *pctldev)
{
	...
	pr_debug("rtk-pinctrl: " "[%s] pctl->ngroups=%d",
...
static int RTK_pinctrl_probe(struct platform_device *pdev)
{
	...
	pr_debug("rtk-pinctrl: " "iso_membase(0x%p)", pctl->iso_membase);

	pctl->misc_membase = of_iomap(node, 1);
	if (!pctl->misc_membase) {
		pr_err("rtk-pinctrl: " "of_iomap misc_membase fail");
		return -ENOMEM;
	}

	pr_debug("rtk-pinctrl: " "misc_membase(0x%p)", pctl->misc_membase);
