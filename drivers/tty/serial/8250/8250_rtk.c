...
static int dw8250_probe(struct platform_device *pdev)
{
	...
	/*
	 * RTD119x DW UART often leaves UCV/CPR unimplemented (read as 0), so
	 * setup_port never enables FIFO and we stay on PORT_8250 (1-byte).
	 * Console writes clear IER while transmitting; without an RX FIFO,
	 * keystrokes during boot/login printk are overrun and lost.
	 * Hardware still has the usual 16-byte DW FIFO — force it.
	 */
	if (!p->fifosize) {
		p->type = PORT_16550A;
		p->flags |= UPF_FIXED_TYPE;
