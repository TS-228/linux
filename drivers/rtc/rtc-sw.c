...
static void sw_rtc_alarm_timeout(struct timer_list *t)
{
    struct sw_rtc_data *data = from_timer(data, t, timer);
...
static int sw_rtc_alarm_start(struct sw_rtc_data *data)
{
    ...
    mod_timer(&data->timer, jiffies + msecs_to_jiffies(delta * 1000));
    return 0;
}
