...
{
    struct rtksd_host *sdport = from_timer(sdport, t, timer);
    u8 wflag = 0;
    u32 int_status = 0;
...
static int mmc_Select_SDR50_Push_Sample(struct rtksd_host *sdport){			
	...
	{
		cr_writeb(0x10, sdport->base_io+SD_PUSH_POINT_CTL);	//Push point =output at falling edge
		sync();
		if ((err = rtkcr_send_cmd25(sdport))==0)
		{
	...
	if ((err = rtkcr_send_cmd18(sdport))==0)
	{
		...
		cr_writeb(8, sdport->base_io+SD_SAMPLE_POINT_CTL);	//sample point is delayed by 1/4 SDCLK period
		sync();
		if ((err = rtkcr_send_cmd18(sdport))==0)
		{
