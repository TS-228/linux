					 ...
					 const u8 *data, size_t count)
{
	...
	if (i < count)
		dev_warn(&serdev->dev, "rx buffer full, dropped %zu bytes\n", count - i);
...
static int qnap_ts228_pic_probe(struct serdev_device *serdev)
{
	...
	/*
	 * No MAC preload. The EEPROM read command gets no reply from this PIC
	 * -- it fails with -ETIMEDOUT and produces no receive callback at all,
	 * the same as the temperature and fan commands did -- so preloading
	 * only spent a second and a half on a doomed transfer and warned about
	 * it on every boot. The cell stays registered; a reader still gets the
	 * honest -ETIMEDOUT.
	 */
	{
