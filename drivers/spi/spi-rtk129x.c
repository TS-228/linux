...
static int rtk_gspi_probe(struct platform_device *pdev)
{
 ...
 /* Use managed reset control API; this tree lacks reset_control_get() */
 struct reset_control *rstc = devm_reset_control_get_exclusive(&pdev->dev, NULL);
 if (IS_ERR(rstc)) {
	dev_dbg(&pdev->dev, "reset control get failed: %ld\n", PTR_ERR(rstc));
	rstc = NULL;
 }
...
static void rtk_gspi_remove(struct platform_device *dev)
{
