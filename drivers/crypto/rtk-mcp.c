 ...
 *
 * Supports AES-128 ECB/CBC on the RTD1195 SoC.
 ...
 *
 * Copyright (c) 2026 Stephan Eicher <stephan.eicher@web.de>
 */
			  ...
			  unsigned int keylen)
{
	...
	/*
	 * Registering as ecb(aes) and cbc(aes) means promising every AES key
	 * size; testmgr refuses a driver whose key range differs from the
	 * generic implementation. The engine only does 128, so the other two
	 * go to a software fallback.
	 */
...
static int rtk_mcp_fallback(struct skcipher_request *req, bool enc)
{
	struct rtk_mcp_ctx *ctx = crypto_skcipher_ctx(crypto_skcipher_reqtfm(req));
...
static int rtk_mcp_ecb_encrypt(struct skcipher_request *req)
{
	...
	if (ctx->keylen != AES_KEYSIZE_128)
		return rtk_mcp_fallback(req, true);
...
static int rtk_mcp_ecb_decrypt(struct skcipher_request *req)
{
	...
	if (ctx->keylen != AES_KEYSIZE_128)
		return rtk_mcp_fallback(req, false);
...
static int rtk_mcp_init_tfm(struct crypto_skcipher *tfm)
{
	...
	ctx->fallback = crypto_alloc_skcipher(name, 0, CRYPTO_ALG_ASYNC |
...
static void rtk_mcp_exit_tfm(struct crypto_skcipher *tfm)
{
	struct rtk_mcp_ctx *ctx = crypto_skcipher_ctx(tfm);
...
static int rtk_mcp_probe(struct platform_device *pdev)
{
	...
	mdev->rst = devm_reset_control_get_optional_exclusive(&pdev->dev, NULL);
	if (IS_ERR(mdev->rst))
	...
	/*
	 * Pulse it. The boot loader leaves the engine out of reset with
	 * nonsense in its ring registers, and deasserting an already
	 * deasserted line does nothing, so every register write was dropped
	 * and the first request timed out.
	 */
	...
	dev_info(&pdev->dev,
		 "Realtek MCP crypto: AES-128 ECB/CBC (ring@%pad)\n",
		 &mdev->ring_dma);
	return 0;
...
MODULE_AUTHOR("Stephan Eicher <stephan.eicher@web.de>");
