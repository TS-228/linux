...
static int rtk_cc_resume(struct device *dev)
{
	...
	for (i = rtk_cc_clock_num() - 1; i >= 0; i--)
...
static int rtk_cc_probe(struct platform_device *pdev)
{
	...
	ccd = devm_cc_alloc(dev, rtk_cc_clock_num());
	if (!ccd)
		return -ENOMEM;
	...
	ret = rtk_cc_init_clocks(dev);
	if (ret)
		dev_err(dev, "rtk_cc_init_clocks() returns %d\n", ret);

	ret = of_clk_add_hw_provider(np, of_clk_hw_onecell_get, ccd->data);
	if (ret)
		dev_err(dev, "of_clk_add_hw_provider() returns %d\n", ret);
