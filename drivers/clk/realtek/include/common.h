...
/**
 * struct clk_reg - a clk core with direct hardware register access
 *
...
static inline struct cc_desc *devm_cc_alloc(struct device *dev, int max_clk)
{
	...
	ccd->data = devm_kzalloc(dev, struct_size(ccd->data, hws, max_clk),
				 GFP_KERNEL);
	if (!ccd->data)
		return NULL;
	ccd->data->num = max_clk;
#ifdef CONFIG_PM
...
/**
 * rtk_cc_init_clocks - platform specific init function, should init data and
 *                  pm_data in cc_desc for platform driver.
 *
 * @dev - device of cc with cc_desc in drvier data
 */
int rtk_cc_init_clocks(struct device *dev);

/**
 * rtk_cc_clock_num - returns clock number in the platform cc
 */
int rtk_cc_clock_num(void);
