...
DEFINE_DEBUGFS_ATTRIBUTE(clk_pll_rate_op,
...
static int clk_pll_conf_show(struct seq_file *s, void *data)
{
	...
	if (pll->ops == &rtk_clk_pll_ops || pll->ops == &rtk_clk_pll_div_ops) {
		seq_puts(s, "freq-tbl-entries:\n");
		for ( ; !IS_FREQ_TABLE_END(ftbl); ftbl++)
			seq_printf(s, "   %10lu : %08x\n", ftbl->rate, ftbl->val);
	}

	if (pll->ops == &rtk_clk_pll_div_ops) {
		struct clk_pll_div *plld = to_clk_pll_div(hw);
		const struct div_table *dtbl = plld->div_tbl;

		seq_puts(s, "div-tbl-entries:\n");
...
{
	debugfs_create_file_unsafe("rtk_clk_rate", 0644, dentry, hw,
				   &clk_pll_rate_op);
	debugfs_create_file("clk_pll_conf", 0444, dentry, hw,
			    &clk_pll_conf_fops);
}
...
/* lowest rate the PLL can generate; the table is not required to be sorted */
static const struct freq_table *ftbl_lowest(const struct freq_table *ftbl)
{
	const struct freq_table *best = NULL;

	for ( ; !IS_FREQ_TABLE_END(ftbl); ftbl++)
		if (!best || ftbl->rate < best->rate)
			best = ftbl;
...
/* entries run from highest rate to lowest, so the last one is the minimum */
static const struct div_table *dtbl_lowest(const struct div_table *dtbl)
{
	const struct div_table *last = dtbl;
			       ...
			       unsigned long *parent_rate)
{
	...
	/*
	 * ftbl_find_by_rate() only ever rounds down, so a request below the
	 * lowest rate the PLL supports finds nothing. Round up to that rate
	 * instead of returning 0, which the core would take as a valid answer
	 * and then feed back in as a 0 Hz set_rate request.
	 */
	ftblv = ftbl_find_by_rate(pll->freq_tbl, rate);
	if (!ftblv)
		ftblv = ftbl_lowest(pll->freq_tbl);
	if (!ftblv)
		return 0;
				   ...
				   unsigned long *parent_rate)
{
	...
	/*
	 * Below the lowest supported rate there is no divider entry. Round up
	 * to that rate rather than returning 0, which the core would take as
	 * a valid answer and then quietly leave the clock where it was.
	 */
	dv = dtbl_find_by_rate(plld->div_tbl, rate);
	if (!dv) {
		dv = dtbl_lowest(plld->div_tbl);
		rate = dv->rate;
	}
				...
				unsigned long parent_rate)
{
	...
	if (WARN(!cdv, "%pC: invalid d-tbl entry for %u", hw->clk, cur_d))
		return -EINVAL;
...
const struct clk_ops rtk_clk_pll_div_ops = {
	.debug_init       = clk_pll_debug_init,
...
bool clk_hw_is_pll(struct clk_hw *hw)
{
	...
	if (init->ops == &rtk_clk_pll_ops ||
	    init->ops == &rtk_clk_pll_div_ops)
		return true;
