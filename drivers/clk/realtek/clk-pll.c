					 ...
					 unsigned long parent_rate)
{
 ...
 if (!fv) {
     /*
      * Some platforms may boot with a PLL configuration value that is
      * not described in the frequency table. Treat it as an unknown but
      * valid hardware state instead of triggering a WARN splat during
      * clock registration. Fall back to the parent (osc) rate so the
      * framework has a sane value and the system can continue booting.
      */
     pr_warn_once("%pC: unknown PLL table entry for %08x, fallback to %lu Hz\n",
              hw->clk, val, parent_rate);
     return parent_rate; /* parent_rate is validated above */
 }
 return fv->rate;
}
