...
static inline int cc_set_clk_cell(struct cc_desc *ccd, int i, struct clk_hw *hw)
{
	if (ccd->data->hws[i]) {
		pr_warn("%s: failed to fill %s to cell %d, used by %s\n",
			__func__,
			clk_hw_get_name(hw), i,
			clk_hw_get_name(ccd->data->hws[i]));
		return -EINVAL;
	}
	ccd->data->hws[i] = hw;
	return 0;
	       ...
	       struct clk_hw *hw)
{
	...
	cc_set_clk_cell(ccd, cc_index, hw);
	if (pm_flags)
		clk_pm_init(hw->clk, &ccd->pm_data[cc_index], pm_flags);
	return 0;
}

static struct clk_hw *clk_reg_create_composite_clk(struct device *dev,
	...
	struct clk_composite_init_data *init)
{
	...
	hw = clk_hw_register_composite(NULL, init->name, init->parent_names,
				       init->num_parents,
			  ...
			  struct clk_composite_init_data *init)
{
	...
	hw = clk_reg_create_composite_clk(dev, &ccd->init_data, init);
	if (IS_ERR(hw))
		return PTR_ERR(hw);
	init->clk = hw->clk;

	clk_hw_register_clkdev(hw, init->name, NULL);
	cc_set_clk_cell(ccd, cc_index, hw);
	/* init clk pm */
