...
CMA_REALLOC:
	...
	list_for_each_entry(phandler, &rtk_carveout_heap->pool_handlers, list) {
		...
		page = cma_alloc(phandler->cpool, size>>PAGE_SHIFT, cma_align, GFP_KERNEL);
...
void ion_rtk_carveout_free(struct ion_heap *heap, phys_addr_t addr,
	unsigned long size)
{
	struct ion_rtk_carveout_heap *rtk_carveout_heap =
		container_of(heap, struct ion_rtk_carveout_heap, heap);
...
static int ion_rtk_carveout_heap_phys(struct ion_heap *heap,
	struct ion_buffer *buffer, phys_addr_t *addr, size_t *len)
{
...
static unsigned long ion_rtk_carveout_heap_gen_pool_algo_last_fit(unsigned long *map,
...
static unsigned long ion_rtk_carveout_heap_gen_pool_algo(unsigned long *map,
	unsigned long size, unsigned long start, unsigned int nr, void *data,
	struct gen_pool *pool, unsigned long start_addr)
{
	struct ion_rtk_carveout_heap *rtk_carveout_heap = data;
	...
	if (rtk_carveout_heap != NULL) {
		...
		case ALGO_FIRST_FIT:
			return gen_pool_first_fit(map, size, start, nr, data,
						  pool, start_addr);
		case ALGO_LAST_FIT:
			return ion_rtk_carveout_heap_gen_pool_algo_last_fit(map,
				size, start, nr, data, pool, start_addr);
		default:
			break;
		}
	}
	...
	return gen_pool_first_fit(map, size, start, nr, data, pool, start_addr);
}
